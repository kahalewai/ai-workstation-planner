...
**APEX-Quant toggle** — MoE-aware mixed-precision weight quantization ([mudler/apex-quant](https://github.com/mudler/apex-quant)). Exploits the structural sparsity of Mixture-of-Experts models to compress weights beyond what uniform quantization achieves. Toggle it on/off to see the weight size difference. Only affects MoE models; dense models show "n/a."

**TurboQuant toggle** — KV cache compression ([TheTom/llama-cpp-turboquant](https://github.com/TheTom/llama-cpp-turboquant)). Compresses the context memory 3–5x using turbo3/turbo4 quantization types. Toggle it on/off to see how your context window grows or shrinks. Includes sub-options for turbo3, turbo4, and asymmetric (K:q8_0 + V:turbo3) configurations.
...
```javascript
{
  ...
  weight_gb_no_apex: 19.7,     // Size without APEX (for toggle comparison)
...
**APEX-Quant** assigns different quantization precision to different tensor types within MoE models — high precision for shared experts and edge layers, aggressive compression for the redundant middle-layer routed experts. This is fundamentally different from uniform quantization like Q4_K_M.
